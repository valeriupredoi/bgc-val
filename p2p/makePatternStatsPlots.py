...
class makePatternStatsPlots:
  ...
  def setDictionaries(self,):
	...
	# metricDicts: List of metrics to load for each shelve.
  	self.metricsDict =  { 'rgam':'Scale Ratio (Data/Reference)',
			 'rE':'Norm. Diff. Scale',
			 'rE0':'Norm. Bias',
			 'rR':'Spearman Corr.',
			 'rP':'p-value (robust)',
	...
	self.keyscolours = {m:i for m,i in zip(self.keys,['r','b',purple,'g','orange','CornflowerBlue','DarkOrchid','DarkTurquoise','FireBrick','LightSeaGreen','Orchid',])}
	
	self.AllShelves = []
	for key,shelves in self.shelveDict.items():self.AllShelves.extend(shelves)
	

  def loadFromshelve(self,sh, sh_key):
  	#####
  	# Simple code to return mask if can't load the shelve
  	try:	return sh[sh_key]
  	except: return np.ma.masked
  ...
  def loadMetrics(self,):
	...
	#	print 'first loop (xkeys):', xkey 
		...
		for key,shelves in self.shelveDict.items():
			...
			sig =  self.loadFromshelve(sh,'robust.gamma')>1 and 1 or -1
			
			metrics['rgam'][xkey][key] = self.loadFromshelve(sh,'robust.gamma')
			metrics['rE0'][xkey][key]  = self.loadFromshelve(sh,'robust.E0') 
			metrics['rE'][xkey][key]  	= self.loadFromshelve(sh,'robust.E') *sig
			metrics['rR'][xkey][key]	= self.loadFromshelve(sh,'robust.R') 			
			metrics['rP'][xkey][key]	= self.loadFromshelve(sh,'robust.p') 						

			sig =  self.loadFromshelve(sh,'Taylor.gamma')>1 and 1 or -1			
			metrics['tgam'][xkey][key] = self.loadFromshelve(sh,'Taylor.gamma')
			metrics['tE0'][xkey][key]  = self.loadFromshelve(sh,'Taylor.E0') 
			metrics['tE'][xkey][key]  	= self.loadFromshelve(sh,'Taylor.E') * sig			
			metrics['tR'][xkey][key]   = self.loadFromshelve(sh,'Taylor.R') 
			metrics['tP'][xkey][key]   = self.loadFromshelve(sh,'Taylor.p') 
			
			
			for k in ['N','b1','b0']:
				metrics[k][xkey][key] = self.loadFromshelve(sh,k)
			
			metrics['TotalModel'][xkey][key] = self.calcTotalConcentration(sh,key='datax')	/1.e12	
			metrics['TotalInSitu'][xkey][key] = self.calcTotalConcentration(sh,key='datay')	/1.e12				
			metrics['Model:In situ'][xkey][key] = metrics['TotalModel'][xkey][key] / metrics['TotalInSitu'][xkey][key]
  ...
  def plotMetrics(self,):
	...
	for plotType,yAxisKeys in self.plotStyles.items():
		...
		for d,metric in enumerate(yAxisKeys):
			...
			linesDict = {'x':[],'xticks':[],'xticklabels':[],'TotalInSitu':[],'MeanInSitu':[],}
			for key in self.keys: linesDict[key] = []			
			...
			if metric == yAxisKeys[-1]: 	
				pyplot.xticks(linesDict['xticks'], linesDict['xticklabels'],rotation = 'vertical',)
			else:	pyplot.xticks(linesDict['xticks'], ['',])
