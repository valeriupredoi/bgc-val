...
#	alwaysInclude = intersection(nci.variables.keys(),alwaysInclude) 
	alwaysInclude = intersection(list(nci.variables.keys()),alwaysIncludList) 
	...
	for var in save:
		...
		if self.debug: 
			print('mergeNC:\tINFO:\tCreating Variable:',var,dt,nci.variables[var].dimensions, end=' ')
	...
	for t,fni in enumerate(self.fnsi):
		...
		for var in list(a.keys()):
		  #if var in time:continue
		  if var in tvars:continue # there may be more than one time variable: ie time and month.		  
		  if var in list(nci.variables.keys()): arr = nci.variables[var][:]
		  else:
