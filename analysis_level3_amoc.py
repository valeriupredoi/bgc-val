...
def analysis_omz(jobID=''):	
	...
	for name in av.keys():
		...
		filename = ukp.folder(imagedir)+'_'.join([name,jobID,])+'.png'				
		...
		if len(modeldata)>30:
			smoothing = tsp.movingaverage2(modeldata,window_len=30,window='hanning',extrapolate='axially')
			pyplot.plot(times,modeldata,      c='b',ls='-',lw=0.2,label='Model1')	
			pyplot.plot(times,smoothing,c='b',ls='-',lw=2,label='Model2')		
		else:
			pyplot.plot(times,modeldata,c='b',ls='-',lw=1,label='Model',)	
