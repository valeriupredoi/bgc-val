        ...
        if year0 in [   'AlignToDECK',	   'HistoricalDECK',	'AlignToDECK1600',
			...
			if year0 in ['AlignToDECK1930','AlignToDECK1600-1930','HistoricalDECK1930']:
				if t1 > 1931.: continue
                        ...
                        if year0 in ['AlignToDECK2100','AlignToDECK1600-2100','HistoricalDECK2100']:
                                if t1 > 2101.: continue

                        if year0 in ['AlignToDECK2150','AlignToDECK1600-2150','HistoricalDECK2150']:
                                if t1 > 2151.: continue

                        if year0 in ['AlignToDECK2200','AlignToDECK1600-2200','HistoricalDECK2200']:
                                if t1 > 2201.: continue
	...
	if year0 in ['ControlAligned',] :
		histruns = {	'u-aw331': 1850, #	UKESM1 first historical member (1850)
				...
				'u-bb277': 2395, #	UKESM1 seventh Historical run (1995)			
	                        'u-bc019': 2250, #      UKESM1 Historical run (2250)
        	                'u-bc041': 2165, #      UKESM1 Historical run(2165)
			   }
...
#                        if jobID in ['u-as462','u-as643',]:    t1 = t  +3312.
   ...
   #                     else: t1=t
        ...
        if year0 in ['ransom2',]:
                if jobID in ['u-am927i','u-am927ii','u-aq853iii','u-aq853']:
                        for t in sorted(mdata.keys()):
                                if jobID in ['u-am927i','u-am927ii','u-aq853iii','u-aq853',]:     t1 = t+2835     # 2130-2230

                                if float(t1) <4945.: continue
                                if jobID in ['u-am927i','u-am927ii',] and t1 > 5315.: continue
