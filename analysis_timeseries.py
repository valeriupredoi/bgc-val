...
#			print "" 
	...
	if 'AMOC' in analysisKeys:
		...
		# Load grid data
		nc = Dataset(orcaGridfn,'r')
		e3v = nc.variables['e3v'][:,LAT-1:LAT+2,:]	# z level height 3D
		e1v = nc.variables['e1v'][LAT-1:LAT+2,:]	# 
		tmask = nc.variables['tmask'][:,LAT-1:LAT+2,:]
		
		nc.close()		
		
		# load basin map
		nc = Dataset('data/basinlandmask_eORCA1.nc','r')
		tmask = e2u = nc.variables['tmaskatl'][LAT-1:LAT+2,:]	# 2D Atlantic mask
		nc.close()		
		...
		def amoc(nc,keys):
			zv = np.ma.array(nc.variables['vomecrty'][:,:,LAT-1:LAT+2,:]) # m/s
