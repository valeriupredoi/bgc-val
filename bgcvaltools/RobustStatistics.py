...
def Sn(data):
    ...
    a2=empty(y.shape)
    a2[0]=y[n//2]-y[0]
    for i,yy in izip(arange(1,nlm),y[1:nlm]):
	nA=i
	nB=n-i-1
	diff=nB-nA
	leftA=1
	...
	if leftA>Amax:
	    ...
	    medB=y[leftB+i]-yy
	    a2[i]=min(medA,medB)
    for i,yy in izip(arange(nlm,n-1),y[nlm:n-1]):
	nA=n-i-1
	nB=i
	diff=nB-nA
	leftA=1
...
def Qn(data):
	...
	j=0
	for i,l,r,yy in izip(arange(1,n),left[1:],right[1:],y[1:]):
	    if l<=r:
		w=r-l+1
		jhelp=l+w//2
		weight[j]=w
	...
	j=0
	for i,yy in izip(arange(n-1,-1,-1),y[::-1]):
	    while j<n and yy-y[n-j-1]<trial:
		j+=1
	    P[i]=j
	j=n+1
	for i,yy in izip(arange(n),y):
...
def _whimed(a,iw):
    ...
    while True:
      ...
      wmid=0
      wright=0
      for i,aa,w in izip(arange(nn),a[:nn],iw[:nn]):
	if aa<trial:
	    wleft+=w
	elif aa>trial:
	    wright+=w
	...
	nn=kcand
	wrest+=wleft+wmid
      for i,aa,w in izip(arange(nn),acand[:nn],iwcand[:nn]):
	a[i]=aa
	iw[i]=w
