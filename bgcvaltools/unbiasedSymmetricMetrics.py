...
def __testMNFBarrays__(model, obs,key):
	...
	if len(model)!= len(obs):
		print key,"calculation:\tModel and data don't match in length!",len(model),'!=', len(obs)
		assert False
...
def MNFB(model, obs):
	...
	model,obs = __testMNFBarrays__(model,obs,'MNFB')
  	...
  	return _np_.ma.sum(si *( _np_.ma.exp(_np_.ma.abs(_np_.ma.log(model/obs))) -_np_.ma.ones(len(obs))))/float(len(obs))
...
def MNAFE(model, obs):
	...
	model,obs = __testMNFBarrays__(model,obs,'MNAFE')
  	return _np_.ma.sum( _np_.ma.abs( _np_.ma.exp(_np_.ma.abs(_np_.ma.log(model/obs))) -_np_.ma.ones(len(obs)) )	)/float(len(obs))
...
def NMBF(model, obs):
	...
	model,obs = __testMNFBarrays__(model,obs,'NMBF')
  	...
  	return s* ( _np_.ma.exp(_np_.ma.abs(_np_.ma.log(model.sum()/obs.sum()))) -1. )
...
def NMAEF(model, obs):
	...
	model,obs = __testMNFBarrays__(model,obs,'NMAEF')
  	s = __calcS__(model,obs)	
  	return _np_.ma.abs(model - obs).sum() / (obs.sum()**((1.+s)/2.) * (model.sum()**((1.-s)/2.)) )
