...
def percentilesPlot(
    ...
    try:
        ...
        print("WARNING: THIS PLOT FAILED:", filename, '(probably beaucse of all masks/ infs./nans)')
    pyplot.close()
...
def trafficlightsPlot(
    ...
    if len(times) == 0 or len(arr) == 0:
        print("trafficlightsPlot:\tWARNING:\tdata or time arrays are empty.", len(
            times), len(arr), title)
        return
    if np.ma.is_masked(arr):
        print("trafficlightsPlot:\tWARNING:\tdata arrays is masked", len(
            times), len(arr), title)
        return
...
def simpletimeseries(
    ...
    if len(times) == 0 or len(arr) == 0:
        print("simpletimeseries:\tWARNING:\tdata or time arrays are empty.", len(
            times), len(arr), title)
        return
    if np.ma.is_masked(arr):
        print("simpletimeseries:\tWARNING:\tdata arrays is masked", len(
            times), len(arr), title)
        return
...
#      w=eval(np.hanning(
    ...
    if len(x) != len(returning):
        print("output array is not the same size as input:\tin:", len(
            x), '\tout:', len(returning))
        assert 0
    ...
    for i, jobID in enumerate(sorted(timesD.keys())):
        ...
        if plotStyle == 'Separate':
            if len(list(timesD.keys())) <= 4:
                ...
                print("Something is wrong here", i, plotStyle, len(
                    list(timesD.keys())), jobID)
            print("Separate plots", i, jobID, len(list(timesD.keys())), len(axs))
            axs[i].set_title(jobID)

        if len(times) == 0 or len(arr) == 0:
            print("multitimeseries:\tWARNING:\tdata or time arrays are empty.", len(
                times), len(arr), title, (i, jobID))
            continue
        if np.ma.is_masked(arr):
            print("multitimeseries:\tWARNING:\tdata arrays is masked", len(
                times), len(arr), title, (i, jobID))
            continue
...
def hovmoellerAxis(fig,
    ...
    if debug:
        print("hovmoellerAxis:\txaxis:", title, xaxis, "\tyaxis:", yaxis, "\tdata:", data)
    if debug:
        print("hovmoellerAxis:\txaxis:", title, xaxis.shape, "\tyaxis:", yaxis.shape, "\tdata:", data.shape)
...
def hovmoellerPlot(modeldata,
    ...
    print("hovmoellerPlot model:", title, md.shape, md.mean(
    ), times.shape, yaxis.shape)  #, times, yaxis
    ...
    if len(dd):
        ...
        print("hovmoellerPlot data: (pre-mask)", title, '\t', dd.shape, dd.min(
        ), dd.mean(), dd.max())
        dd = np.ma.masked_where(np.ma.masked_invalid(dd).mask + dd.mask, dd)
        print("hovmoellerPlot data: (post-mask)", title, '\t', dd.shape, dd.min(
        ), dd.mean(), dd.max())
    ...
    print("hovmoellerPlot: - data:", title, dd.shape, dd.mean(
    ), dxaxis.shape, dyaxis.shape)
